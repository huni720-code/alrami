  ...
  return (
    <Layout>
      ...
      <div className="flex items-center gap-3 mb-8">
        <h2 className="text-2xl font-bold text-gray-800">관리자 페이지</h2>
        <span className="bg-emerald-100 text-emerald-700 text-xs font-bold px-2.5 py-1 rounded-full tracking-wide">
          ADMIN
        </span>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="flex border-b border-gray-100">
          {(['cards', 'telecom'] as const).map(tab => (
            ...
            >
              {tab === 'cards'
                ? `💳 저장된 카드 (${savedCards.length})`
                : `📡 저장된 요금제 (${savedTelecom.length})`}
            </button>
          ))}
        </div>
        <div className="p-4">
...
function CrawlSection({
  ...
  const icon = target === 'cards' ? '💳' : '📡'
  const source = target === 'cards' ? '카드고릴라 + 폴백' : '스마트초이스 + 통신3사 + 폴백'

  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
      {/* 타이틀 + 상태 */}
      <div className="flex items-start justify-between mb-1">
        <h3 className="font-semibold text-gray-800 text-lg">{icon} {label} 크롤링</h3>
        {task && (
      ...
      <p className="text-xs text-gray-400 mb-4">
        출처: {source}
        {task?.completed_at && (
          <> · 마지막: {new Date(task.completed_at).toLocaleString('ko-KR')}</>
        )}
      </p>

      {/* 에러 메시지 */}
      {task?.error_message && (
        <div className="bg-red-50 border border-red-100 rounded-lg p-3 mb-4 text-xs text-red-600">
...
function ResultPreview({ target, results }: { target: string; results: any[] }) {
  ...
  const crawled = results.filter(r => r.source === 'crawled' || r.source === 'smartchoice' || r.source === 'cardgorilla')
          ...
          <tbody className="divide-y divide-gray-50">
            {valid.map((item, i) => (
              <tr key={i} className="hover:bg-gray-50/80">
                {target === 'cards' ? (
                  <>
                    <td className="px-3 py-1.5 text-gray-700 max-w-[160px] truncate">{item.name}</td>
                  ...
                  <>
                    ...
                    <td className="px-3 py-1.5 text-gray-700 max-w-[160px] truncate">{item.plan_name}</td>
...
function SourceBadge({ source }: { source: string }) {
  const isCrawled = source === 'crawled' || source === 'smartchoice' || source === 'cardgorilla'
  return (
    <span className={`px-1.5 py-0.5 rounded text-xs ${
      isCrawled ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-500'
    }`}>
      {isCrawled ? '크롤링' : '기준데이터'}
    </span>
  )
...
function CardsTable({ cards, onDelete }: { cards: CardRow[]; onDelete: (id: number) => void }) {
  ...
  if (cards.length === 0) {
    return <p className="text-center text-gray-400 text-sm py-10">저장된 카드 데이터가 없습니다. 크롤링 후 DB 저장을 눌러주세요.</p>
  }
  return (
              ...
              >
                ...
                <td className="py-2 px-2">
                  <span className={`text-xs px-2 py-0.5 rounded-full ${
                    card.card_type === 'credit' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600'
                  }`}>
                ...
                <td className="py-2 px-2 text-center" onClick={e => e.stopPropagation()}>
                  <button onClick={() => onDelete(card.id)} className="text-red-400 hover:text-red-600 text-xs transition-colors">
                    삭제
                  </button>
                </td>
              </tr>
                        ...
                        <tbody className="divide-y divide-blue-50">
                          {card.benefits.map((b, i) => (
                            <tr key={i} className="text-gray-600">
                              ...
                              <td className="py-1.5 px-2 text-gray-600 max-w-[260px]">{b.description || '-'}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
...
function TelecomTable({ plans, onDelete }: { plans: TelecomRow[]; onDelete: (id: number) => void }) {
  if (plans.length === 0) {
    return <p className="text-center text-gray-400 text-sm py-10">저장된 요금제 데이터가 없습니다. 크롤링 후 DB 저장을 눌러주세요.</p>
  }
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
          ...
          {plans.map(plan => (
            <tr key={plan.id} className="hover:bg-gray-50 transition-colors">
              ...
              <td className="py-2 px-2 text-center">
                <button onClick={() => onDelete(plan.id)} className="text-red-400 hover:text-red-600 text-xs transition-colors">
                  삭제
                </button>
              </td>
            </tr>
